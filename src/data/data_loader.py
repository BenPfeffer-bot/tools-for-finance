...
class DataLoader:
    ...
    def load_data(
        ...
        try:
            ...
            logger.info(
                f"Downloading data for {self.tickers} from {start_date.date()} to {end_date.date()}"
            )
            ...
            if df.empty:
                logger.warning(f"No data retrieved for {self.tickers}")
                return None

            logger.info(f"Successfully loaded data for {len(self.tickers)} tickers.")
            ...
            logger.error(f"Error loading data for {self.tickers}: {str(e)}")
            return None
    ...
    def run(
        ...
        # Save if requested
        if save:
            file_name = f"{self.tickers[0]}_{pd.Timestamp.now().strftime('%Y%m%d')}.csv"
